...
class PlannerAgent:
    ...
    async def review(self, state: PlanExecuteState) -> dict[str, Any]:
        ...
        correlation_id = state.get("correlation_id", "default")
        progress = {
            "total_steps": int(state.get("total_steps", len(plan) + len(state.get("past_steps", [])))),
            "completed_step_count": int(state.get("completed_step_count", len(state.get("past_steps", [])))),
            "remaining_step_count": len(plan),
        }
        ...
        updates: dict[str, Any] = {"next_agent": "diagnosis", **progress}
        ...
        if decision.action == "replan" and decision.revised_steps:
            ...
            updates["remaining_step_count"] = len(revised)
