...
FALLBACK_PLAN_STEPS = [
    "使用 query_prometheus_alerts 查询当前活跃告警并确认告警状态",
    "根据已确认告警查询对应服务的监控指标和错误日志",
    "基于工具返回的证据生成诊断报告；如无活跃告警则明确说明系统当前无告警",
]


def _resolve_plan_steps(plan_result: Any) -> List[str]:
    """Extract validated steps from ChatQwen's include_raw response."""
    if isinstance(plan_result, dict) and "parsed" in plan_result:
        parsing_error = plan_result.get("parsing_error")
        if parsing_error is not None:
            logger.warning(f"Planner 结构化输出解析失败，使用安全降级计划: {parsing_error}")
            return list(FALLBACK_PLAN_STEPS)
        plan_result = plan_result.get("parsed")
    ...
    return normalized_steps or list(FALLBACK_PLAN_STEPS)
...
async def planner(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    try:
        ...
        planner_chain = planner_prompt | llm.with_structured_output(Plan, include_raw=True)
        ...
        plan_steps = _resolve_plan_steps(plan_result)
        ...
        # 返回一个默认计划
        return {"plan": list(FALLBACK_PLAN_STEPS)}
