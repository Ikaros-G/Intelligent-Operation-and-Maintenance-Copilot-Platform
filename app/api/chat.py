...
async def chat_stream(request: ChatRequest):
        ...
        try:
            async for chunk in rag_agent_service.query_stream(request.question, session_id=request.id):
                ...
                if chunk_type == "debug":
                    ...
                    # 发送检索结果（可选，前端可以忽略）
                    yield {"type": "search_results", "data": chunk_data}
                elif chunk_type == "content":
                    # 发送内容块 - 关键：data 必须是 JSON 字符串
                    yield {"type": "content", "data": chunk_data}
                elif chunk_type == "complete":
                    # 发送完成信号
                    yield {"type": "done", "data": chunk_data}
                elif chunk_type == "error":
                    # 发送错误信息
                    yield {"type": "error", "data": str(chunk_data)}
    ...
    stream_id = await resumable_stream_service.start("chat", request.id, event_generator)
