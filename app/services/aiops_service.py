...
class AIOpsService:
    ...
    async def execute(
        ...
        try:
            ...
            WORKFLOW_DURATION.labels(status="success").observe(time.perf_counter() - started)
    ...
    async def resume(self, session_id: str, approval: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        async for event in self.graph.astream(Command(resume=approval), config=config_dict, stream_mode="updates"):
            for node_name, node_output in event.items():
    ...
    def _format_planner_event(self, state: Dict | None) -> Dict:
        ...
        return {
            "type": "plan",
            "stage": "plan_created",
            "message": f"执行计划已制定，共 {len(public_plan)} 个步骤",
            "plan": public_plan
        }
    ...
    def _format_replanner_event(self, state: Dict | None) -> Dict:
        ...
        return {
            ...
            "message": f"Planner Agent 复核完成，{'继续诊断' if state.get('next_agent') == 'diagnosis' else '转交报告 Agent'}",
            "remaining_steps": len(plan)
        }
