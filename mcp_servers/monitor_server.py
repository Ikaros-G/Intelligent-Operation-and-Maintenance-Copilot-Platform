...
def restart_service(service_name: str, reason: str) -> Dict[str, Any]:
    ...
    这是高风险工具，只能由 Copilot 的管理员审批工作流调用。示例环境不会执行
    操作系统命令，而是返回一个可审计的重启受理记录；生产环境应将此函数替换为
    受认证的编排平台 API，并继续保留上层审批和权限校验。
    """
    allowed_services = {"data-sync-service", "api-gateway-service", "payment-service"}
    if service_name not in allowed_services:
        raise ValueError("服务未纳入可操作白名单")
    if not reason.strip():
    ...
    return {
        ...
        "service_name": service_name,
        "reason": reason,
        "operation_id": f"restart-{int(datetime.now().timestamp())}",
        "message": "示例环境已记录重启请求，未执行系统命令",
    }
